...
Version 0.4.0 targets VS Code 1.106+ and follows the layered plan in `CLAUDE.md`: data → providers → feature modules → entry point. Activation now relies solely on the built-in providers, so new language features should continue to integrate with those layers. Interpreter, package, dependency, template, and task managers boot lazily via `ensurePhase6Initialized()`; reuse that helper when adding commands to keep startup lean.
