...
The debugger regression suite was exercised on macOS with Tcl 8.5.9 and Thread 2.6.6. Its 19 tests cover source/argument preservation, selected-frame inspection and edits, conditional breakpoints and logpoints, stepping, authenticated attach and failure handling, source mapping, detach, independent workers, and generated-source cleanup. Remote path mapping was tested through local fixtures; a live connection to a remote host through SSH was not exercised. Other Tcl/Thread/platform combinations require their own runtime validation.
