...
/**
 * Run the active TCL script after prompting for a discovered interpreter.
 */
export async function runWithInterpreter(interpreterManager: TclInterpreterManager): Promise<void> {
...
export async function runWithArgs(interpreterManager: TclInterpreterManager): Promise<void> {
    const editor = getActiveTclEditor();
    if (!editor) {
        return;
    }
 ...
 *
 * Call this only when commands are not already proxied by `extension.ts`.
 */
export function registerRunCommands(
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('tcl.runWithInterpreter', () => runWithInterpreter(interpreterManager)),
        vscode.commands.registerCommand('tcl.runWithArgs', () => runWithArgs(interpreterManager))
    );
}
